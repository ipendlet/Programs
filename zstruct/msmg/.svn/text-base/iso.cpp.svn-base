...
//  for (int i=0;i<NMAXSTRUCTS;i++)
//    isosr[i] = new double[N3];
  double* energies = new double[NMAXSTRUCTS];
  double* energiesr = new double[NMAXSTRUCTS];

  geom1r.reset(natoms,geoms[id].anames,geoms[id].anumbers,geoms[id].coords);
  geom1r.ic_create();

  Mopac mopt;
  mopt.alloc(natoms);
  int* frzlist = new int[natoms*2];
  int nfrz = 0;
  int nfrz0 = 0;

  mopt.reset(natoms,geoms[id].anumbers,geoms[id].anames,geoms[id].coords);
...
//    printf("\n\n looking for moves for atom %i \n",i);
  ...
  int** addsubmm  = new int*[NMAXSTRUCTS+1];
  int** addsubpp  = new int*[NMAXSTRUCTS+1];
  int** addsubmp  = new int*[NMAXSTRUCTS+1];
  int** addsubmp2 = new int*[NMAXSTRUCTS+1];
  for (int i=0;i<NMAXSTRUCTS+1;i++)
...
//////////////////
  ...
  for (int j=0;j<i;j++)
  {
    if (!geoms[id].bond_exists(i,j) && ncmovesp[i] && ncmovesp[j]
        && !geoms[id].hpair(i,j))
    {
      movesp[np][0] = i;
      movesp[np][1] = j;
  ...
  for (int i=0;i<np;i++)
  {
    ...
    geom1.reset(natoms,geoms[id].anames,geoms[id].anumbers,geoms[id].coords);
    geom1.ic_create();
    //geom1.print_bonds(); 
...
//    done = geom1.opt(optfile);
    ...
    if (done)
    {
      ...
      mopt.reset(geom1.natoms,geom1.anumbers,geom1.anames,geom1.coords);
      mopt.freeze(frzlist, nfrz, nfrz0);
      mopt.opt_write(optfile,geom1);
      tniso++;
      for (int l=0;l<N3;l++) isos[tniso][l] = mopt.xyz[l];
    }
    else { nfail++; nscratch--; }
...
//         printf(" with atom moving: %i \n",mov1);
         if (geoms[id].bond_exists(mov1,addsubmp[i][0]) || 
...
//         printf(" with atom detaching: %i \n",mov1);
      ...
      if (!skip)
      {
        ...
        if (!geoms[id].bond_exists(k,addsubmp[i][0]) && !geoms[id].hpair(k,addsubmp[i][0]))
        {
          mov2=k;
          printf("\n bond to add: %i %i to remove: %i %i\n",addsubmp[i][0],mov2,
                 geoms[id].bonds[rem1][0],geoms[id].bonds[rem1][1]);
...
//        for (int k2=0;k2<k1;k2++)
        {
          ...
          if (geoms[id].bond_exists(mov1,a1) || geoms[id].bond_exists(mov2,a2)) skip = true;
...
//        for (int k2=0;k2<k1;k2++)
        {
          ...
          if (rem1==rem2) skip = true;
          if (geoms[id].bond_exists(mov1,a2) || geoms[id].bond_exists(mov2,a1)) skip = true;
...
#if PPMM
  printf("\n doing 2x +/- moves which have one unsat. or sat. center +/- \n");
  for (int i=0;i<nasmp;i++)
  {
          ...
          && !frozen[a2] )
      {
        ...
        for (int k2=0;k2<geoms[id].nbonds;k2++)
        {
         ...
         //case one: a1 to c4, a2/c2 to c3
          ...
          {
            int skip1 = false;
            if (geoms[id].bond_exists(a1,c4) || geoms[id].bond_exists(a2,c3)) skip1 = true;
       ...
       //case two a1 to c3, a2/c2 to a3, c4 disconnects (doesn't follow disconnect to connect scheme)
          if (a1!=c3 && ncmovesm[c4] && !skip)
          {
            for (int k=0;k<natoms;k++)
            {
              a3 = k;
              int skip1 = false;
              if (geoms[id].bond_exists(a1,c3) || geoms[id].bond_exists(a2,a3)) skip1 = true;
              if (a2==a3) skip = true;
...
#if PPMM
  printf("\n\n doing 2x +/- moves which have two unsat. or sat. centers +/- \n");
  for (int i=0;i<natoms;i++)
  {
    for (int j=0;j<i;j++)
    {  
     ...
     // a1/c1 and a2/c2 must be min or max coord, non hydrogen
            ...
            && !frozen[a1] )
      {
        ...
        for (int k2=0;k2<geoms[id].nbonds;k2++)
        {
          ...
          if (geoms[id].bond_exists(a1,c4) || geoms[id].bond_exists(a2,c3)) skip = true;
...
//        if (a2==c2) skip = true;
        if (geoms[id].bond_exists(a13,a2)) skip = true;
...
// third case for +/- at one center with one minus move
  ...
  for (int i=0;i<natoms;i++)
  {
    ...
    for (int j=0;j<natoms;j++)
    {
      if (i!=j && !geoms[id].bond_exists(i,j) && !frozen[j])
      {
        for (int k1=0;k1<geoms[id].nbonds;k1++)
        {
        //labels are wrong!
...
//    printf(" in mp pair: %i %i \n",addsubmp[i][0],addsubmp[i][3]);
    ...
    { 
      ...
      skip = false;
      a3 = j; a4 = k;
      if (geoms[id].bond_exists(a3,a4)) skip = true;
...
//      printf(" ncmovesp[a3]: %i ncmovesp[a4] %i \n",ncmovesp[a3],ncmovesp[a4]);
      ...
      for (int k1=0;k1<geoms[id].nbonds;k1++)
      {
        ...
        if (geoms[id].bonds[k1][0]==c1 || geoms[id].bonds[k1][1]==c1)
        {
          ...
          if (geoms[id].bond_exists(a1,mov1)) skip = true;
...
//    printf(" central atom for +/-: %i \n",i);
    ...
    { 
      ...
      skip = false;
      a3 = j; a4 = k;
      if (geoms[id].bond_exists(a1,a3)) skip = true;
      if (!ncmovesp[a3]) skip = true;
      if (!ncmovesp[a4]) skip = true;
...
//      printf(" ncmovesp[a3]: %i ncmovesp[a4] %i \n",ncmovesp[a3],ncmovesp[a4]);
      ...
      for (int k1=0;k1<geoms[id].nbonds;k1++)
      {
        ...
        if (geoms[id].bonds[k1][0]==c1 || geoms[id].bonds[k1][1]==c1)
        {
          ...
          if (geoms[id].bond_exists(mov1,a4)) skip = true;
          if (mov1==a1) skip = true;
          if (mov1==a3) skip = true;
          if (mov1==a4) skip = true;
...
*/
  ...
  {
    geom1.reset(natoms,geoms[id].anames,geoms[id].anumbers,isos[i]);
    geom1.ic_create();
...
//    geoms[i+niso].init(natoms,geoms[id].anames,geoms[id].anumbers,isos[order[i+1]]);
    geoms[i+niso].reset(natoms,geoms[id].anames,geoms[id].anumbers,isos[order[i+1]]);
