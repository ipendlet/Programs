...
int GeomBasis::init(string xyzlist){
 ...
 printf(" in GeomBasis, reading in xyz for ICoord \n");
 natomt = 0;
...
// for (int i=0;i<1;i++)
 for (int i=0;i<nbasis;i++)
 {
    ...
    for (int j=0;j<icoord[i].natoms;j++)
    {
      ...
      printf(" new atom (%s/%i) coordination #: %i \n",tanam[0].c_str(),tanum[0],k-1);
      ...
      if (found==0)
      {
        ats[natomt].alloc(natt);
        ats[natomt].lic.skipTMadd = 1;
        ats[natomt].reset(natt,tanam,tanum,txyz);
        natomt++;
      }
...
// will need check to differentiate between two metal types
...
//    return -1;
  ...
  for (int i=0;i<natomt;i++)
  {
    ...
    if (anum==icoord.anumbers[n])
    {
      ...
      {
//detecting H attached to B or C (extend later)
        int hb = -1;
        for (int j=0;j<icoord.nbonds;j++)
        {
          if (icoord.bonds[j][0]==n)
          {
            if (icoord.anumbers[icoord.bonds[j][1]]==5 || icoord.anumbers[icoord.bonds[j][1]]==6 || icoord.anumbers[icoord.bonds[j][1]]==17)
              hb = icoord.bonds[j][1];
          }
          else if (icoord.bonds[j][1]==n)
          {
            if (icoord.anumbers[icoord.bonds[j][0]]==5 || icoord.anumbers[icoord.bonds[j][0]]==6 || icoord.anumbers[icoord.bonds[j][1]]==17)
              hb = icoord.bonds[j][0];
          }
          if (hb>-1)
          {
...
int GeomBasis::nminus(int n, ICoord icoord, int atype0){
  ...
  int type = -1;
  if (icoord.coordn[n]<2 && !icoord.anumbers[n]==17)
    return -1;
