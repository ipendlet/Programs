...
void ICoord::print_ic()
{
   ...
   for (int i=0;i<nbonds;i++)
      printf(" bond %i: %i to %i: %1.2f \n",i+1,bonds[i][0],bonds[i][1],bondd[i]);
   printf("\n");
   ...
   //printf(" ntmlinangles: %i \n",ntmlinangles);
#if 1
//   for (int i=0;i<nangles;i++)
   for (int i=0;i<4;i++)
   {
      printf(" angle %i: %i %i %i: %1.1f ",i+1,angles[i][0],angles[i][1],angles[i][2],anglev[i]);
      for (int l=0;l<ntmlinangles;l++)
      if (tmlinangles[l][0]==angles[i][0] && tmlinangles[l][2]==angles[i][2])
      {  printf("*");  break;  }
      else if (tmlinangles[l][2]==angles[i][0] && tmlinangles[l][0]==angles[i][2])
      {  printf("*");  break;  }
      printf("\n");
   }
   printf("\n");
